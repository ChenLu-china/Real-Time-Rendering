...
template<typename T>
struct SumOp {
  device forceinline T operator()(const T& a, const T& b) const { return a + b; }
};

template<typename T>
struct MaxOp {
  device forceinline T operator()(const T& a, const T& b) const { return max(a, b); }
};

template<typename T>
struct MaxIndexOp {
  device forceinline T operator()(const T& a, const T& b, const T&idx_x, const T&idx_y) const { 
    if(a > b){
      return idx_x;
    }
    else{
...
inline device T WarpAllReduce(T val) {
...
template<template<typename> class ReductionOp, template<typename> class IndexOp, typename T, int thread_group_width = kWarpSize>
inline device T WarpAllFindIndex(T val, T idx) {
  for (int mask = thread_group_width / 2; mask > 0; mask /= 2) {
    idx = IndexOp<T>()(val, __shfl_xor_sync(0xffffffff, val, mask), idx, __shfl_xor_sync(0xffffffff, idx, mask));
    val = ReductionOp<T>()(val, __shfl_xor_sync(0xffffffff, val, mask));
  }
  return idx;
}
...
inline device T BlockAllReduce(T val) {
  typedef cub::BlockReduce<T, block_size> BlockReduce;
  shared typename BlockReduce::TempStorage temp_storage;
  shared T result_broadcast;
...
template<>
inline device float Inf<float>() {
  return CUDART_INF_F;
}

template<>
inline device double Inf<double>() {
  return CUDART_INF;
}

// 定义 exp 函数，OF_SOFTMAX_USE_FAST_MATH 这个宏表示开启 FAST_MATH 的 exp
template<typename T>
inline device T Exp(T x);

template<>
inline device float Exp<float>(float x) {
...
template<>
inline device double Exp<double>(double x) {
  return exp(x);
}

// 定义 div 函数，OF_SOFTMAX_USE_FAST_MATH 这个宏表示开启 FAST_MATH 的 div
template<typename T>
inline device T Div(T a, T b);

template<>
inline device float Div<float>(float a, float b) {
#ifdef OF_SOFTMAX_USE_FAST_MATH
...
template<>
inline device double Div<double>(double a, double b) {
  return a / b;
}
...
template<>
inline device float Log<float>(float x) {
#ifdef OF_SOFTMAX_USE_FAST_MATH
  return __logf(x);
#else
  return log(x);
...
template<>
inline device double Log<double>(double x) {
  return log(x);
}
...
#pragma unroll
  ...
  device void change(int64_t row, int64_t col, float value) {
    const int64_t location = row * row_size + col - 1;
    *(src + location) = value;
  }
  SRC* src;
  int64_t row_size;
...
// template<typename SRC, typename DST>
// struct DirectResult{
//   DirectResult(DST* dst, int64_t row_size) : dst(dst), row_size(row_size) {}
// };


template<typename SRC, typename DST>
struct DirectStore {
  DirectStore(DST* dst, int64_t row_size, int64_t rows, DST* max, DST* prb, DST* x, int64_t* max_col) : dst(dst), row_size(row_size), rows(rows), max(max), prb(prb), x(x), max_col(max_col) {}
  template<int N>
  device void store(const SRC* src, int64_t row, int64_t col) {
    Pack<DST, N> pack;
    const int64_t offset = (row * row_size + col) / N;
#pragma unroll
...
// 和store.template store<pack_size>(ptr, row_id, col_id); 进行读取和写入
...
// 每个线程分配rows_per_access * cols_per_thread大小的寄存器，将输入x读到寄存器中，后续计算均从寄存器中读取。
...
global void SoftmaxWarpImpl(LOAD load, STORE store, const int64_t rows, const int64_t cols) {
  ...
  for (int64_t row = global_thread_group_id * rows_per_access; row < rows; row += step) {
    ...
    // 开辟一块共享内存记录当前线程组处理每一行的最大值索引
    ComputeType thread_index_max[rows_per_access];
#pragma unroll
    for (int row_id = 0; row_id < rows_per_access; ++row_id) {
      // 把 thread_max[row_id] 初始化为 -inf
      thread_max[row_id] = -Inf<ComputeType>();
      thread_index_max[row_id] = -Inf<ComputeType>();
      // 获取第 row_id 行的共享内存数据
...
#pragma unroll
    for (int row_id = 0; row_id < rows_per_access; ++row_id){
      float mininf = -Inf<ComputeType>();
      load.change(row + row_id, cols, mininf);
      // 把 thread_max[row_id] 初始化为 -inf
      thread_max[row_id] = -Inf<ComputeType>();
      thread_index_max[row_id] = -Inf<ComputeType>();
      // 获取第 row_id 行的共享内存数据
...
template<typename LOAD, typename STORE, typename ComputeType, int pack_size, int block_size,
         Algorithm algorithm>
global void SoftmaxBlockSMemImpl(LOAD load, STORE store, const int64_t rows,
                                     const int64_t cols) {
  extern shared __align__(sizeof(double)) unsigned char shared_buf[];
...
template<typename LOAD, typename STORE, typename ComputeType, int pack_size, int block_size,
         Algorithm algorithm>
global void SoftmaxBlockUncachedImpl(LOAD load, STORE store, const int64_t rows,
                                         const int64_t cols) {
  const int tid = threadIdx.x;
  assert(cols % pack_size == 0);
  const int num_packs = cols / pack_size;
...
template<typename LOAD_Y, typename LOAD_DY, typename STORE, typename ComputeType, int pack_size,
         ...
         Algorithm algorithm>
global void SoftmaxGradWarpImpl(LOAD_Y load_y, LOAD_DY load_dy, STORE store, const int64_t rows,
                                    const int64_t cols) {
  static_assert(cols_per_thread % pack_size == 0, "");
...
global void SoftmaxGradBlockSMemImpl(LOAD_Y load_y, LOAD_DY load_dy, STORE store,
                                         const int64_t rows, const int64_t cols) {
  extern shared __align__(sizeof(double)) unsigned char grad_shared_buf[];
...
template<typename LOAD_Y, typename LOAD_DY, typename STORE, typename ComputeType, int pack_size,
         int block_size, Algorithm algorithm>
global void SoftmaxGradBlockUncachedImpl(LOAD_Y load_y, LOAD_DY load_dy, STORE store,
                                             const int64_t rows, const int64_t cols) {
  const int tid = threadIdx.x;
  assert(cols % pack_size == 0);
  const int num_packs = cols / pack_size;
...
void runtest(float* input_host, 
  ...
  float* prb_device; 
  float* x_device;
  cudaMalloc((void **)& MaxCol_device, rows * sizeof(int64_t));
  ...
  DirectStore<ComputeType, float> store(output_device, cols, rows, max_device, prb_device, x_device, MaxCol_device);
