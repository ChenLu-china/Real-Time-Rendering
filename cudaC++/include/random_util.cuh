...
struct Normal{
    Normal() = default;
    __device__ Normal(const float loc, const float scale):loc(loc), scale(scale){}
    __device__ void set(const float loc, const float scale){
        this->loc = loc;
        this->scale = scale;
    }

    __host__ __device__ 
    float log_prob(const float value){
        float var = scale * scale;
        float log_scale = log(scale);
        float log_prob = -((value - loc) * (value - loc)) / (2 * var) - log_scale - log(sqrt(2 * M_PI));
        return log_prob;
    }
    float loc;
    float scale; 
